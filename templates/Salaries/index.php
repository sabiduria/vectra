        ...
        <table id="scroll-vertical" class="table table-bordered text-nowrap w-100 TableData">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-end">
                        <?= $this->Html->link(__('<i class="ri-eye-line"></i>'), ['action' => 'view', $salary->id], ['class' => 'btn btn-success btn-sm', 'escape' => false]) ?>
                        <?= $this->Html->link(__('<i class="ri-pencil-line"></i>'), ['action' => 'edit', $salary->id], ['class' => 'btn btn-primary btn-sm', 'escape' => false]) ?>
                        <?= $this->Form->postLink(__('<i class="ri-delete-bin-line"></i>'), ['action' => 'delete', $salary->id], ['class' => 'btn btn-danger btn-sm', 'confirm' => __('Voulez-vous supprimer cette information ?'), 'escape' => false]) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
...
<div class="offcanvas offcanvas-end" tabindex="-1" id="NewItem"
    ...
    <div class="offcanvas-header border-bottom border-block-end-dashed">
        <h5 class="offcanvas-title" id="offcanvasRightLabel1">Nouveau Salaries</h5>
...
<script>
    $(document).ready(function() {
        $('#DataForm').on('submit', function(e) {
            ...
            $.ajax({
                url: '<?= $this->Url->build(["controller" => 'Salaries', 'action' => 'insert']) ?>',
                method: 'POST',
                data: formData,
